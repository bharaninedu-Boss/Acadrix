...
(function(){
  ...
  async function mount(container,code){
    ...
    host.innerHTML=`<h2>📚 PDF Study Library</h2><div class="pdf-library-note"><strong>PDF-first:</strong> Choose a category below. Each category reads its own GitHub folder automatically. No JSON editing is needed for PDFs. <a class="pdf-upload" href="${githubFolderUrl(folder)}" target="_blank" rel="noopener">Open subject PDF folders →</a></div><nav class="pdf-category-tabs" aria-label="PDF resource categories">${CATEGORIES.map(c=>`<button type="button" class="pdf-tab" data-category="${c.id}">${c.icon} ${c.title} <span class="pdf-tab-count">…</span></button>`).join('')}</nav><div id="pdfCategories">Loading resources…</div>`;
